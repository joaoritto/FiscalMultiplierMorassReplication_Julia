...
## Computing Lig-Likelihood
Likelihood_Kalman<-function( TTT,R,Q,Z,H,W,data) {
  
  
  dimS=dim(TTT)
  dimS=dimS[1]
  
  Filtering=Kalm_f(TTT,R,Q,Z,H,W,data)
  vt=Filtering[[5]]
  Ft=Filtering[[6]]
  
  # Output
  Total_lik=0
  Lik=matrix(rep(0,TT),TT,1)
  ...
  for (t in 1:TT) {
    ...
    if (abs(det(F))<1e-14) {
      ...
      invF=solve(F)
      Lik[t,1]=-1/2*log(abs(det(F)))-1/2*t(v)%*%invF%*%v
    }
  }
  
  Total_lik=-0.5*dimS*TT*log(2*pi)+sum(Lik)
  return(Total_lik)
}
...
LogLikelihood1 = Likelihood_Kalman( TTT,R,Q,Z,H,W,data)
